...
public class EditorMain {
    ...
    private EditorMain() {
        ...
        e
            .getAssetManager()
            .getRef(e.getAssetManager().registerAsset(Paths.get("out/")));
        e.setEditor(false); //TODO:???

        @SuppressWarnings({ "unused", "unchecked" })
        ImUI ui = new ImUI(e, () -> {
            //Update Windows
            for (ImWindow w : rwins) wins.remove(w);
            for (ImWindow w : wins) w.display();
            ...
            if (ImGui.begin("Asset Status")) {
                ...
                for (Entry<UUID, Asset> a : s) {
                    ImGui.text(
                        a.getValue().getReferences() +
                            ":" +
                            e.getAssetManager().getPath(a.getKey())
                    );
                }
            }
            ImGui.end();
            ...
            if (ImGui.begin("Scene Panel")) {
                for (int i = e.em.getEntities().size() - 1; i >= 0; i--) {
                    ...
                    if (ent.equals(selected)) ImGui.smallButton(
                        ent.getName().isEmpty()
                            ? "Unnamed Entity" + ent.hashCode()
                            : ent.getName()
                    );
                    else ImGui.text(
                        ent.getName().isEmpty()
                    ...
                    if (ImGui.isItemClicked(1)) {
                        ...
                        if (ent.equals(selected)) selected = null;
                    }
                    if (ImGui.isItemClicked()) selected = ent;
                }
            }
            ImGui.end();
            ...
            if (ImGui.begin("Renderer Stats")) {
                ImGui.text(
                    "Dimensions:" +
                        e.getFinalBuffer().getWidth() +
                        "x" +
                        e.getFinalBuffer().getHeight()
                );
                e
                    .getSystems()
                    .forEach(a -> {
                        //a bit questionable but will get reworked down the line (Vulkan multithreading)
                        if (a instanceof SRenderer) ImGui.text(
                            "Drawcalls: " + ((SRenderer) a).getDrawcalls()
                        );
                    });

                ImGui.text("Frametime: " + frametime.getmillis());
                ImGui.text(
                    e.em.getEntities().size() + " Entities currently loaded"
                );
                ImGui.text(
                    e.getAssetManager().getAssets().size() +
                        " Assets currently loaded"
                );
                ...
                if (ImGui.collapsingHeader("Time")) {
                    if (ImPlot.beginPlot("RenderTime")) {
                        for (Entry<String, List<Long>> entry : map.entrySet()) {
                            ...
                            if (entry.getValue().size() > 0) ImPlot.plotLine(
                                entry.getKey(),
                                ys
                                    .subList(0, entry.getValue().size())
                                    .toArray(new Long[entry.getValue().size()]),
                                entry
                                    .getValue()
                                    .toArray(new Long[entry.getValue().size()])
                            );
            ...
            if (ImGui.begin("Test")) {
                try {
                    e
                        .getPostProcessor()
                        .serialize(new ImGuiSerializeAdapter(this));
            ...
            if (ImGui.begin("Scene")) {
                ...
                if (
                    windowsize.x != e.getFinalBuffer().getWidth() ||
                    windowsize.y != e.getFinalBuffer().getHeight()
                ) e.setBufferSize(windowsize.x, windowsize.y);

                int renderid = e.getFinalBuffer().getAttachments()[0].getID();
                if (showpost) renderid = e.getFinalImage().getID();
                if (showocc) renderid = e
                    .getFinalBuffer()
                    .getAttachments()[1].getID();
                ...
                if (ImGui.isItemHovered()) {
                    for (EntityInteractionDriver eid : eid) {
                        for (int mb = 0; mb < 3; mb++) {
                            if (ImGui.isMouseClicked(mb)) eid.onClick(
                                mousex,
                                mousey,
                                mb
                            );
                            if (ImGui.isMouseDown(mb)) eid.onDown(
                                mousex,
                                mousey,
                                mb
                            );
        ...
        //Timed Update
        e.update(
            new MultipleTimer().setOnReset(a -> {
                for (Entry<String, Long> entr : a) {
                    if (!map.containsKey(entr.getKey())) map.put(
                        entr.getKey(),
                        new LinkedList<Long>()
                    );
                    List<Long> l = map.get(entr.getKey());
                    l.add(entr.getValue());
                    if (l.size() > 100) l.remove(0);
                }
            })
        );
        System.exit(0);
    ...
    private void renderMenuBar(Engine e) {
        if (ImGui.beginMainMenuBar()) {
            if (ImGui.beginMenu("File")) {
                ...
                if (
                    ImGui.menuItem("Save", "Ctrl + S", false, savepath != null)
                ) {
                    try {
                        IOUtils.write(
                            this.play ? this.currscene : e.em.saveScene(),
                            savepath
                        );
                    } catch (IOException e1) {
                        e1.printStackTrace();
                ...
                if (ImGui.menuItem("Save As", "Ctrl + Shift + S")) {
                    savepath = EditorUtils.showFileDialog(
                        AssetManager.getAssetManager().getPath()
                    );
                    if (savepath != null) try {
                        if (!savepath.endsWith("scene")) savepath = Paths.get(
                            savepath.toAbsolutePath().toString() + ".scene"
                        );
                        IOUtils.write(
                            this.play ? this.currscene : e.em.saveScene(),
                            savepath
            ...
            if (ImGui.beginMenu("Scene")) {
                if (
                    ImGui.menuItem("Show PostProcessing", "", showpost)
                ) showpost = !showpost;
                ImGui.endMenu();
            }
            if (ImGui.beginMenu("Scene")) {
                if (ImGui.menuItem("Show Occluder", "", showocc)) showocc =
                    !showocc;
                ImGui.endMenu();
            }
    ...
    private void inspectEnt(Entity selected) {
        ...
        if (ImGui.beginMenu("Add Component")) {
            for (@SuppressWarnings("rawtypes")
            Class c : Engine.getEngine().em.getRegisteredComponents())
